...
export const YarnWorkspacesTests: AcceptanceTests = {
  ...
  tests: {
    ...
    'test --yarn-workspaces-alt --detection-depth=5': (params, utils) => async (
      t,
    ) => {
      utils.chdirWorkspaces();
      const result = await params.cli.test('yarn-workspaces-alt', {
        yarnWorkspaces: true,
        detectionDepth: 5,
      });
      const loadPlugin = sinon.spy(params.plugins, 'loadPlugin');
      ...
      t.match(
        result.getDisplayResults(),
        'Tested 2 dependencies for known issues, found 2 issues, 2 vulnerable paths.',
        'correctly showing dep number',
      );
      t.match(result.getDisplayResults(), 'Package manager:   yarn\n');
      t.match(
