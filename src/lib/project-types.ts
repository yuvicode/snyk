...
export type SupportedBuildTool =
  | 'sbt'
  | 'gradle'
  | 'maven';

export type SupportedProjectTypes = SupportedPackageManagers | SupportedBuildTool | 'docker';
export type SupportedProjectTypesWithManifest = SupportedPackageManagers | SupportedBuildTool;
...
function getManifestFiles(packageManager: string): string[] {
  return get(PROJECT_TYPES_WITH_MANIFEST, [packageManager, 'manifestFiles'], []);
}

function getDefaultManifestFile(packageManager: string): string | null {
  return get(PROJECT_TYPES_WITH_MANIFEST, [packageManager, 'defaultManifestFile'], null);
}
