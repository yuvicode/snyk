...
import { test } from '../../../lib/snyk-test-v2';
...
export async function testV2(root: string,
  ...
  // TODO: support project name override from config
  ...
  for (let i = 0; i < res.length; i++) {
    results.push(_.assign(res[i], {root}));
  ...
  const vulnerableResults = results.filter((ress) => ress.vulnerabilities && ress.vulnerabilities.length);
  const errorResults = results.filter((ress) => ress instanceof Error);
